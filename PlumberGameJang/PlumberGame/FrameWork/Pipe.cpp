...
{
	type = type1; //미리보여지는 파이프를 현재 파이프로 하나씩 밀어넣는다??
	type1 = type2;
	type2 = type3;
	type3 = Type();
	
	rotate_num = rotate1; //로테이션 할때마다 블록이 변경??
	rotate1 = rotate2;
	rotate2 = rotate3;
	rotate3 = Type();

	if (type == 3) // 4번째 돌리면 0번으로
	{
		rotate_num = 0;
	}
}
		...
		if (eventblock.valvecheck == true)
		{
			if (GetTickCount() - waterupdate > 150)
			{
				...
				for (int i = 0; i < 9; i++)
				{
					for (int k = 0; k < 9; k++)
					{						
						if (save[i][k].WaterMax == true)		//해당 파이프에 물이 다 차잇을시
						{
							if (save[i][k].WaterCount == 2)		//물이 흐르는 스프라이트 계속 움직이게
							{
								save[i][k].WaterCount = 0;
							}
							else
						...
						if (save[i][k].MaxWaterCount >= 6)		//물이 다 찻나 검사후 다 찻으면 다찻다고 알림
						{
...
void Pipe::Draw()
{
	...
	{		
		for (int i = 0; i < 9; i++)
		{
			for (int k = 0; k < 9; k++)
			{
				if (save[i][k].isPipe == true)		//해당위치에 파이프가 놓여잇으면 저장된정보로 그려준다
				{					
					p_pipe[save[i][k].type][save[i][k].rotate].Render((k * BLOCKSIZE + BACKSTART), (i * BLOCKSIZE + BACKSTART), 0, 1, 1);
				}
				if (save[i][k].isWater == true)			//해당위치 파이프에 물이 들어오면 물을 그려준다
				{
		...
		p_pipe[type][rotate_num].Render(m_W, m_H, 0, 1, 1);		// 마우스로 움직일수잇는 파이프를 그려준다		
	}
}
...
void Pipe::PipeMove(LPARAM lParam)
{
	...
	for (int i = 0; i < 9; i++)
	{
		for (int k = 0; k < 9; k++)
		{
				...
				HIWORD(lParam) > (i * BLOCKSIZE + BACKSTART))
			{
				...
				{
					m_W = (k * BLOCKSIZE + BACKSTART);		//해당위치에 고정
	...
	if (check == 0)		// 블럭을 놓을수잇는 공간에 들어가잇지 않을시
	{
		...
		for (int k = 0; k < 9 ; k++)
		{
				...
				HIWORD(lParam) > (i * BLOCKSIZE + BACKSTART))
			{
				...
				{					
					m_W = (k * BLOCKSIZE + BACKSTART);		//해당위치에 고정
					m_H = (i * BLOCKSIZE + BACKSTART);
					save[i][k] = { true, type,rotate_num };		//그려줄 정보 저장

					AddPipe(PIPE,i * 3 + 1, k * 3 + 1, rotate_num, type);		//백그라운드 배열에 파이프배열 저장
					
					return 1;		//파이프가 잘 놓여졋다고 전달
				}
				else
				{
					return 0;		//파이프가 안 놓여졋다고 전달
				}
			}
		}
	}
...
{
	//해당 파이프 위치에 물이 들어왓을떄 물이 들어왓다고 켜줌
	for (int i = 1; i < 28; i++)
	{
		for (int k = 1; k < 28; k++)
		{
